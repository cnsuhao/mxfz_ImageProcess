...
void CFFTDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Radio(pDX, IDC_AMPLITUDE, m_ShowMode);
	DDX_CBString(pDX, IDC_FILTER, m_Filter);
	DDX_Text(pDX, IDC_PARAM1, m_Param1);
	DDX_Text(pDX, IDC_PARAM2, m_Param2);
	DDX_Control(pDX, IDC_PARAM1, m_Param1_Con);
	DDX_Control(pDX, IDC_PARAM2, m_Param2_Con);
...
BEGIN_MESSAGE_MAP(CFFTDlg, CDialogEx)
	ON_WM_CREATE()
	ON_WM_PAINT()
	ON_BN_CLICKED(IDC_AMPLITUDE, &CFFTDlg::OnBnClickedAmplitude)
	ON_BN_CLICKED(IDC_ANGLE, &CFFTDlg::OnBnClickedAngle)
	ON_EN_CHANGE(IDC_PARAM1, &CFFTDlg::OnEnChangeParam1)
	ON_EN_CHANGE(IDC_PARAM2, &CFFTDlg::OnEnChangeParam2)
	ON_CBN_SELCHANGE(IDC_FILTER, &CFFTDlg::OnCbnSelchangeFilter)
	ON_BN_CLICKED(FFTCANCEL, &CFFTDlg::OnBnClickedFftcancel)
