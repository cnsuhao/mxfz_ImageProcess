...
void CCurveDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_MOUSE_X, m_X);
	DDX_Text(pDX, IDC_MOUSE_Y, m_Y);
	DDX_Text(pDX, IDC_EDIT_X, m_d_X);
	DDX_Text(pDX, IDC_EDIT_Y, m_d_Y);
}


BEGIN_MESSAGE_MAP(CCurveDlg, CDialogEx)
	ON_WM_PAINT()
	ON_CBN_SELCHANGE(IDC_COLOR_SELECT, &CCurveDlg::OnCbnColorSelect)
	ON_WM_CREATE()
	ON_WM_MOUSEMOVE()
	ON_WM_LBUTTONUP()
	ON_BN_CLICKED(IDOK, &CCurveDlg::OnBnClickedOk)
	ON_BN_CLICKED(IDCANCEL_CURVE, &CCurveDlg::OnBnClickedCancel)
	ON_WM_CLOSE()
	ON_BN_CLICKED(IDC_BTN_ADD_POINT, &CCurveDlg::OnBnClickedBtnAddPoint)
	ON_BN_CLICKED(IDC_BTN_CLEAR, &CCurveDlg::OnBnClickedBtnClear)
END_MESSAGE_MAP()
